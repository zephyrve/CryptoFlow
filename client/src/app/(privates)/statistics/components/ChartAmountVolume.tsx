...
const CustomTooltipAmount: React.FC<TooltipProps<any, any>> = ({active, payload, label}) => {
            ...
            <div className="custom-tooltip" style={{
                ...
                {payload.map((entry, index) => (
                    <p key={`item-${index}`} style={{color: entry.color}}>
                        {`${entry.name?.split('-').join(' ')}: ${entry.value} BTT`}
                    </p>
                ))}
            </div>
        );
...
export const TransactionCountChart: React.FC = () => {
    ...
    const prepareChartData = (transactionHistory: Transaction[]) => {
        ...
        // @ts-ignore
        const sortedTransactionHistory = [...transactionHistory].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));

        const chartData: { [key: string]: ChartData } = {};
        // @ts-ignore
        const transactionTypes = [...new Set(sortedTransactionHistory.map(tx => tx.type))];

        sortedTransactionHistory.forEach(({type, amount, timestamp}) => {
